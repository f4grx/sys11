...
static uint8_t sci_read(void *ctx, uint16_t off)
  {
    ...
    switch(off)
      ...
      case OFF_SCCR1: printf("SCI read BAUD: %02X\n", sci->regs[off]); break;
      case OFF_SCCR2: printf("SCI read BAUD: %02X\n", sci->regs[off]); break;
      case OFF_SCSR: printf("SCI read BAUD: %02X\n", sci->regs[off]);  break;
      case OFF_SCDR: printf("SCI read BAUD: %02X\n", sci->regs[off]);  break;
      }
    return ret;
  }
